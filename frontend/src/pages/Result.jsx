  ...
  return (
    ...
    <div>
      <div className='relative'>
        <img src={image} alt="sample 1" className='max-w-sm rounded'/>
        <span className={`absolute bottom-0 left-0 h-1 bg-blue-500 w-full transition-all duration-[10s] ${isLoading ? 'w-full transition-all duration-[10s]' : 'w-0'}`}/>
        <p className={!isLoading ?'hidden' : '' }>Loading...</p>
      </div>
    </div>
      {!isImageLoaded && 
      <div className='flex w-full max-w-xl bg-neutral-500 text-white mt-10 p-0.5 rounded-full'>
        <input
      ...
      <div className='flex gap-2 flex-wrap justify-center text-white text-sm p-0.5 mt-10 rounded-full'> 
        ...
        onClick={() => {setImageLoaded(false)}}>Generate another</p>
